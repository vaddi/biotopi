 ...
 * http(s)://APP/?controller=protocoltypes&action=read
 * http(s)://APP/?controller=protocoltypes&action=read&id=1
 * http(s)://APP/?controller=protocoltypes&action=create&name=hum0&type=1&threshold=100&protocol=1
 * http(s)://APP/?controller=protocoltypes&action=read&id=2
 * http(s)://APP/?controller=protocoltypes&action=delete&id=2
 * http(s)://APP/?controller=protocoltypes&action=update&id=1&name=hum1
 */

class Protocoltypes implements CRUD {
	 ...
	 */
	public function __construct( $_db, $id, $name, $data ) {
		try {
	...
	private function _save() {
		...
		try {
			if( null !== $this->_params->id ) {
				// update a entry
				$this->_validateParam( 'id' );
				$this->_db->query( "UPDATE $this->_dbTable SET name = :name, data = : data, updated = :updated WHERE id = :id;" );
				$this->_db->bind( ':id', $this->_params->id );
				...
				$this->_validateParam( 'data' );
				$this->_db->query( "INSERT INTO $this->_dbTable ( name, data, created ) VALUES ( :name, :data, :created )" );
				$this->_db->bind( ':created', date( 'Y-m-d H:i:s' ) );
			}
			// update and insert uses the same bindings
			$this->_db->bind( ':name', $this->_params->name );
			$this->_db->bind( ':data', $this->_params->data );
			$this->_db->execute();
			if( null === $this->_params->id ) {
