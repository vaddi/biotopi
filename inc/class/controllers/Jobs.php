...
class Jobs {
	 ...
	 */
	public function __construct( $_db, $id, $name, $device, $type, $start, $stop, $created, $cmd ) {
		try {
			// first param is allways the DB object
			$this->_db = $_db;
			// if $_dbTable is empty use Classname as Tablename
			if( empty( $this->_dbTable ) ) $this->_dbTable = strtolower( __CLASS__ ) . '_v';
			...
			$this->_params->stop = ( isset( $stop ) && $stop !== null ) ? $stop : null;
