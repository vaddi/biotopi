 ...
 * http(s)://APP/?controller=daemontypes&action=read
 * http(s)://APP/?controller=daemontypes&action=read&id=1
 * http(s)://APP/?controller=daemontypes&action=create&name=hum0&type=1&threshold=100&protocol=1
 * http(s)://APP/?controller=daemontypes&action=read&id=2
 * http(s)://APP/?controller=daemontypes&action=delete&id=2
 * http(s)://APP/?controller=daemontypes&action=update&id=1&name=hum1
 */

class Daemontypes implements CRUD {
	 ...
	 */
	public function __construct( $_db, $id, $name ) {
		try {
	...
	private function _save() {
		...
		try {
			if( null !== $this->_params->id ) {
				// update a entry
				$this->_validateParam( 'id' );
				$this->_db->query( "UPDATE $this->_dbTable SET name = :name, updated = :updated WHERE id = :id;" );
				$this->_db->bind( ':id', $this->_params->id );
