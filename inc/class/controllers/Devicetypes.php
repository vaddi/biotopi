 ...
 * http(s)://APP/?controller=devicetypes&action=read
 * http(s)://APP/?controller=devicetypes&action=read&id=1
 * http(s)://APP/?controller=devicetypes&action=create&name=hum0&type=1&threshold=100&protocol=1
 * http(s)://APP/?controller=devicetypes&action=read&id=2
 * http(s)://APP/?controller=devicetypes&action=delete&id=2
 * http(s)://APP/?controller=devicetypes&action=update&id=1&name=hum1
 */

class Devicetypes implements CRUD {
	 ...
	 */
	public function __construct( $_db, $id, $name, $subtype ) {
		try {
			...
			$this->_params->subtype = ( isset( $subtype ) && $subtype !== null ) ? $subtype : null;
		} catch( Exception $e ) {
	...
	public function update() {
		...
		try {
			...
			$this->_params->subtype = null === $this->_params->subtype ? $new['subtype'] : $this->_params->subtype;
			$result = $this->_save();
		} catch( Exception $e ) {
			if( ENV == 'prod' ) throw new Exception( $e->getMessage() );
				else throw new Exception( __CLASS__ . '::' . __FUNCTION__ . ' throw ' . $e->getMessage() );
	...
	private function _save() {
		...
		try {
			if( null !== $this->_params->id ) {
				// update a entry
				$this->_validateParam( 'id' );
				$this->_db->query( "UPDATE $this->_dbTable SET name = :name, subtype = :subtype, updated = :updated WHERE id = :id;" );
				$this->_db->bind( ':id', $this->_params->id );
				...
				$this->_validateParam( 'subtype' );
				$this->_db->query( "INSERT INTO $this->_dbTable ( name, subtype, created ) VALUES ( :name, :subtype, :created )" );
				$this->_db->bind( ':created', date( 'Y-m-d H:i:s' ) );
			}
			// update and insert uses the same bindings
			$this->_db->bind( ':name', $this->_params->name );
			$this->_db->bind( ':subtype', $this->_params->subtype );
			$this->_db->execute();
			if( null === $this->_params->id ) {
