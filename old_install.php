...
<body>
	...
	<table cellspacing="0">
		...
		<tr>
			...
			<?php if (preg_match('|^http(s)?://[a-z0-9-]+(.[a-z0-9-]+)*(:[0-9]+)?(/.*)?$|i', URL)): ?>
				<?php $fcheck = 'install.php'; if ( urlExists( URL . "/" . $fcheck ) ): ?>
		...
		<tr>
			...
			<?php $fcheck = $_SERVER['DOCUMENT_ROOT'] . "" . $_SERVER['REQUEST_URI'] . 'install.php' ; if (is_file( $fcheck )): ?>
	...
	<?php else: ?>
		...
		<p>Remove or rename the <b>install.php</b> file.<br />
		<br />
	<?php endif ?>
